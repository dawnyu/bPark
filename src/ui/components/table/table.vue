<template>
  <div :class="wrapClasses" :style="styles">
    <div :class="classes">
      ...
      <div :class="[prefixCls + '-body',isScroll?'iScroll':'']" :id="name" :style="bodyStyle" ref="body" @scroll="handleBodyScroll" v-show="!((!!noDataText && (!data || data.length === 0)) || (!!noFilteredDataText && (!rebuildData || rebuildData.length === 0)))">
        <table-body ref="tbody" :prefix-cls="prefixCls" :styleObject="tableStyle" :name='name' :columns="cloneColumns" :data="rebuildData" :columns-width="columnsWidth" :obj-data="objData"></table-body>
      </div>
...
export default {
  ...
  methods: {
    ...
    clickCurrentRow(_index) {
      ...
      this.$emit('on-row-click', JSON.parse(JSON.stringify(this.cloneData[_index])), _index, this.name);
    },
    ...
    start() {
      var vm = this
      var _this = $(`#${this.name}`), Interval = ''
      if (_this.length === 0) {
        _this = $($(".ivu-table-body")[0])
      }
      Interval = setInterval(() => {
        if (_this.find(vm.docType).height() <= _this.height()) {
          clearInterval(setInterval);
        } else {
          this.marquee(_this, vm.rowHeight);
        }
      }, vm.speed);
    ...
    marquee(obj, step) {
      var vm = this, rowspan = ~~this.rowspan
      rowspan = rowspan === 0 ? 1 : rowspan
      obj.find(this.docType).animate({
        marginTop: '-=1'
      }, 0, function () {
        var top = $(this).css("margin-top")
        if (top) top = top.replace("px", '')
        var s = Math.abs(~~top)
        if (s >= step * rowspan) {
          $(this).find(vm.subDocTpye).slice(0, rowspan).appendTo($(this));
