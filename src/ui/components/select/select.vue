<template>
  <div :class="classes" v-clickoutside="handleClose">
    <div :class="[prefixCls + '-selection']" ref="reference" @click="toggleMenu">
      <Icon size="20" v-if="type === 'search'" type="ios-search" :class="[prefixCls + '-search-icon']"></Icon>
      <div class="ivu-tag" v-if="!noSelection" v-for="(item, index) in selectedMultiple">
        <span class="ivu-tag-text">{{ item.label }}</span>
        <Icon type="ios-close-empty" @click.native.stop="removeTag(index)"></Icon>
      </div>
      <span :class="[prefixCls + '-placeholder']" v-show="showPlaceholder && !filterable && !noSelection">{{ placeholder }}</span>
      <span :class="[prefixCls + '-selected-value']" v-show="!showPlaceholder && !multiple && !filterable">{{ selectedSingle }}</span>
      <input type="text" v-if="filterable" v-model="query" :class="[prefixCls + '-input']" :placeholder="showPlaceholder ? placeholder : ''" :style="inputStyle" @blur="handleBlur" @keydown="resetInputState" @keydown.delete="handleInputDelete" ref="input">
      <Icon type="ios-close" :class="[prefixCls + '-arrow']" v-show="showCloseIcon" @click.native.stop="clearSingleSelect"></Icon>
      <Icon type="arrow-down-b" :class="[prefixCls + '-arrow']"></Icon>
    </div>
    <transition name="slide-up">
      <Drop v-show="visible" ref="dropdown" :placement="placement">
        <ul v-show="notFound" :class="[prefixCls + '-not-found']">
          <li>{{ notFoundText }}</li>
        </ul>
        <p class="select-full" v-if="noSelection">
          <span :class="{'select-full-btn':!fullSelect,'select-full-btn-selected':fullSelect}" @click.stop="fillSelect">全选</span>
        </p>
  ...
  computed: {
    ...
    showPlaceholder() {
      ...
      if ((typeof this.model) === 'string') {
        ...
        if (this.model.length && this.noSelection) {
          status = true;
        }
  ...
  methods: {
    fillSelect() {
      if (this.fullSelect) {
        ...
        this.model = this.fillCurPark.slice(0)
      }
    },
    toggleMenu() {
      if (this.disabled) {
    ...
    toggleMultipleSelected(value, init = false) {
      if (this.multiple) {
        ...
        if (this.noSelection) {
          if (this.fillCurPark.length === hybridValue.length) {
            this.fullSelect = true
          } else {
            this.fullSelect = false
          }
